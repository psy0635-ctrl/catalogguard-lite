...
FastAPI는 클라이언트의 `X-Request-ID`를 재사용하지 않고 요청마다 새 ID를 생성해 모든 응답의 `X-Request-ID` 헤더에 넣습니다. 애플리케이션은 요청별 구조화 로그를 직접 기록하며, 운영 문제를 추적할 때는 응답에서 이 값을 확인한 뒤 Railway 로그에서 같은 `request_id`를 검색합니다.

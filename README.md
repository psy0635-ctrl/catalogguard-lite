...
상품 카탈로그 CSV를 업로드해 누락, 형식 오류, 중복, 가격 이상치, 카테고리 불일치, 금지어와 개인정보 의심 정보를 검사하고, 검수 결과를 저장·검색·조회·다운로드하는 Streamlit + FastAPI 기반 MVP입니다. 일반 상품 검수 기능에 더해 패션 상품의 색상·사이즈 표기를 권장 표준값과 비교합니다.
...
패션 색상·사이즈 검수는 `core.rules`가 현재 입력값을 `core.fashion_attribute_validator`의 별칭 사전과 비교해 주의 항목을 만듭니다. 권장 표준값은 결과표에만 표시하며 원본 DataFrame, 마스킹 미리보기의 색상·사이즈, `Product.color`, `Product.size`는 변경하지 않습니다.
...
| `core/fashion_attribute_validator.py` | 패션 색상·사이즈 별칭을 권장 표준값으로 찾는 정확 일치 사전과 순수 함수 |
...
오탐을 줄이기 위해 `MELANGE GRAY`, `OATMEAL`, `DUSTY PINK` 같은 사용자 정의·복합 색상과 `95`, `100`, `28`, `30` 같은 숫자 사이즈는 이번 버전에서 판단하지 않습니다. 별칭 사전에 없는 값도 오류나 주의로 만들지 않습니다. 이는 정상값을 잘못 경고하는 일을 줄이기 위한 보수적인 MVP 방식입니다.
...
| `inspection_version` | `String(20)`, nullable 아님 | 검수 규칙 버전입니다. 현재 `INSPECTION_VERSION` 값은 `"2"`입니다. DB `server_default`는 없습니다. |
...
색상·사이즈 표준화 규칙이 추가되면서 같은 CSV의 검수 결과가 달라질 수 있어 현재 `INSPECTION_VERSION`을 `"2"`로 올렸습니다. 동일 CSV라도 검수 버전이 다르면 새로운 검수 결과로 저장할 수 있습니다. DB 스키마 변경은 없어 이 기능을 위한 Alembic migration은 추가하지 않았으며, 과거 이력과 기존 migration의 `"1"` backfill 값은 그대로 유지합니다.
...
```text
632 passed, 25 skipped, 0 failed, 1 warning
```

현재 환경에는 `TEST_DATABASE_URL`이 설정되지 않아 PostgreSQL 연결 및 저장 통합 테스트 25개는 skipped 처리되었습니다. warning 1건은 `.pytest_cache` 쓰기 권한과 관련된 `PytestCacheWarning`이며 기능 오류와 관련된 warning은 아닙니다. GitHub Actions에서는 PostgreSQL 테스트 DB가 제공되므로 이 테스트도 모두 실행됩니다.
...
- 사용자 정의·복합 색상과 숫자 사이즈는 이번 버전에서 판단하지 않습니다.

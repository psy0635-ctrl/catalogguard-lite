...
이 코드가 배포되면 FastAPI는 클라이언트의 `X-Request-ID`를 재사용하지 않고 요청마다 새 ID를 생성해 모든 응답의 `X-Request-ID` 헤더에 넣습니다. 운영 문제를 추적할 때는 응답에서 이 값을 확인한 뒤 Railway 로그에서 같은 `request_id`를 검색합니다.
...
Railway Healthcheck Path는 계속 `/health`로 유지합니다. 배포 후 `/health`, `/ready`, `X-Request-ID` 응답 헤더와 Railway의 한 줄 JSON 로그를 차례로 확인합니다.
...
```text
543 passed, 25 skipped, 1 warning
```

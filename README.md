...
- 현재 테스트 기준: 371개 자동 테스트 통과, skipped 7건, 경고 1건
...
| `db/repositories.py` | 검수 실행과 검수 결과 저장 Repository |
| `db/persistence_service.py` | 검수 결과 저장 트랜잭션 관리 Service |
...
현재 단계에서는 테이블, DB 연결 기반, 저장 Repository와 Service가 구성되어 있습니다. 검수 결과 조회 기능은 아직 구현되지 않았습니다.

현재 저장 Repository와 Service는 구현되어 있지만,
POST /api/v1/inspections 엔드포인트와의 연결은 다음 단계에서 진행합니다.
...
- `db/repositories.py`: SQLAlchemy `Session`을 받아 `inspection_runs`와 `inspection_results`에 객체를 추가하고 `flush`로 ID를 확보합니다. Repository는 `commit`하지 않습니다.
- `db/persistence_service.py`: 파일명을 안전하게 정리하고, 검수 요약과 결과 목록을 하나의 트랜잭션으로 저장합니다. 저장 중 오류가 나면 전체 작업이 rollback되어 일부 결과만 남지 않습니다.

저장 대상은 기존 검수 결과 DataFrame의 안전한 표시 컬럼입니다. 개인정보 관련 오류 이유는 기존 마스킹 결과를 그대로 사용하며, 원본 CSV나 원문 개인정보는 저장하지 않습니다.
...
- 현재 프로젝트는 인증, 검수 결과 DB 저장 API 연결, 검수 기록 조회 API, 외부 API 연동을 포함하지 않습니다.

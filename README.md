...
- 가방의 "별도 사이즈 없음"은 `size=FREE` 입력 규약으로 표현합니다. 카테고리별로 필수 값을 다르게 적용하는 기능은 구현되어 있지 않습니다.

...
같은 Streamlit 세션에서 이미 저장한 CSV를 다시 저장하려고 하면 파일 바이트의 SHA-256 해시를 기준으로 중복 저장을 막고 기존 실행 ID를 안내합니다. 이 처리는 현재 Streamlit 세션 안에서만 적용됩니다.
...
검수 결과를 저장할 때는 Streamlit이 FastAPI에 원본 업로드 파일을 다시 보내고, FastAPI가 같은 검수 로직을 실행한 뒤 PostgreSQL에 저장합니다.
...
| `api/main.py` | FastAPI 앱 생성, 라우터 등록, `/health` 등록 |
| `api/routes/inspections.py` | 검수 생성, 검수 이력 목록, 검수 상세 조회 API |
| `api/schemas.py` | API 응답 Pydantic 모델 |
| `config/settings.py` | CSV 컬럼, 허용 카테고리, 업로드 제한, 금지어, API 클라이언트 환경변수 |
...
| `db/models.py` | `inspection_runs`, `inspection_results` SQLAlchemy 모델 |
| `db/repositories.py` | 검수 실행과 상세 결과 저장·조회 Repository |
| `db/persistence_service.py` | 검수 결과 저장 트랜잭션, 목록 조회, 상세 조회 Service |
| `db/session.py` | SQLAlchemy 엔진, 세션 팩토리, FastAPI 세션 의존성 |
...
현재 마이그레이션 `20260703_0001_create_inspection_tables.py`는 다음 테이블을 만듭니다.
...
- 파일 필드명: `file`
- 정상 응답: `inspection_run_id`, `summary`, `results`
- 잘못된 CSV: HTTP `400`
- 파일 필드 누락: HTTP `422`
...
검수 이력 목록 API와 Streamlit 검수 이력 탭은 파일명 검색을 지원합니다.
...
- 상세 화면에서 목록으로 돌아오면 검색어와 현재 페이지 offset은 유지됩니다.
...
```text
452 passed, 18 skipped
```

`TEST_DATABASE_URL`이 없는 환경에서는 일부 PostgreSQL 통합 테스트가 skipped 처리됩니다.

## 23. 데이터 저장 범위와 보안

PostgreSQL에는 검수 실행 요약과 표시용 상세 결과만 저장합니다.
...
- 인증과 관리자 권한 관리 추가
- FastAPI와 PostgreSQL의 별도 배포 환경 구성
- 검수 이력 삭제, 기간 검색, 상태별 검색 기능 추가
- 검수 이력 화면 스크린샷 문서화
...
- Streamlit의 중복 저장 방지는 현재 세션 기준입니다. 전체 DB 기준 중복 방지가 필요하면 별도 정책을 설계해야 합니다.

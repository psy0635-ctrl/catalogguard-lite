...
이 결과는 PostgreSQL 통합 테스트를 포함한 전체 테스트 결과입니다. warning은 기능 실패가 아니라 `.pytest_cache` 디렉터리 생성 권한과 관련된 `PytestCacheWarning`입니다. `TEST_DATABASE_URL`이 없는 환경에서는 일부 PostgreSQL 통합 테스트가 skipped 처리될 수 있습니다.
...
- 날짜 범위 검색은 구현되어 있지 않습니다.
- 상태별 검수 이력 검색은 구현되어 있지 않습니다.
...
- 전체 검수 이력을 한 번에 CSV로 다운로드하는 기능은 구현되어 있지 않습니다.

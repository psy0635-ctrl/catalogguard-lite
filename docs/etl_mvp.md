...
ETL batch는 profile semantic definition만 담은 optional `profile_definition_snapshot`과 그 SHA(`profile_definition_sha256`)를 저장하며, optional `application_commit_sha`는 실행 application Git commit lineage로 별개입니다. 알 수 없는 경우와 legacy row는 `NULL`이고 backfill하지 않습니다. 이 값들은 Docker image digest나 dependency/runtime 전체 snapshot이 아니므로 완전한 실행 재현성을 뜻하지 않습니다.

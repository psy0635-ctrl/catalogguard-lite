...
| 최신 CI 자동 테스트 결과 | 611 passed in 3.62s, 0 skipped, 0 failed |

## 6.6 핵심 구현 구조

| 파일 | 역할 |
|---|---|
| `app.py` | Streamlit UI와 전체 검수 흐름 연결, 공통 통계 UI 표시 |
...
통계 집계 함수에는 정렬, 빈 값 처리, 필수 컬럼 검증, 입력 불변성과 TOP 5 적용 위치를 확인하는 신규 테스트 14개를 추가했습니다. 로컬에서는 `TEST_DATABASE_URL`을 설정하지 않은 상태로 전체 테스트를 실행해 다음 결과를 확인했습니다. 이때 PostgreSQL 연결 및 저장 통합 테스트 25개는 skipped 처리됩니다.

```text
611 collected
586 passed, 25 skipped, 0 failed, 0 warnings
pytest 실행 시간: 2.21초
```
...
최신 GitHub Actions CI에서는 Alembic 마이그레이션 적용 후 `611 passed in 3.62s`, `0 skipped`, `0 failed`를 확인했습니다. 로컬에서 skipped됐던 PostgreSQL 통합 테스트 25개도 CI에서는 마이그레이션이 적용된 테스트 DB를 사용해 모두 실행됩니다.
...
CI는 테스트 전에 `20260703_0001`, `20260705_0002` Alembic 마이그레이션을 적용합니다. 최신 CI에서는 로컬 단위 테스트와 PostgreSQL 연결·저장 통합 테스트를 합친 전체 611개가 skipped와 실패 없이 통과했습니다.
...
CatalogGuard Lite는 상품 운영자가 CSV 업로드만으로 상품 데이터 품질을 확인할 수 있는 검수 앱입니다. 업로드 검증, 원본 보존형 개인정보 마스킹 미리보기, 중복 상품 탐지, 가격 이상치 탐지, 상품명과 카테고리 불일치 탐지, 필터와 독립된 전체 결과 통계, 결과 필터링, CSV 다운로드까지 하나의 흐름으로 구성했습니다. PostgreSQL에 검수 결과를 저장하고 검색 조건별 이력 조회, 상세 확인과 CSV 다운로드까지 이어지도록 구현했습니다. GitHub Actions CI에서 PostgreSQL 통합 테스트를 포함한 전체 자동 테스트 611개를 실행했으며 모두 통과했습니다.

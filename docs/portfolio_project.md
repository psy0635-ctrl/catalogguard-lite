...
CatalogGuard Lite는 상품 운영자가 CSV 업로드만으로 상품 데이터 품질을 확인할 수 있는 검수 앱입니다. 업로드 검증, 원본 보존형 개인정보 마스킹 미리보기, 중복 상품 탐지, 가격 이상치 탐지, 상품명과 카테고리 불일치 탐지, 결과 필터링, CSV 다운로드까지 하나의 흐름으로 구성했습니다. PostgreSQL에 검수 결과를 저장하고 검색 조건별 이력 조회, 상세 확인과 CSV 다운로드까지 이어지도록 구현했습니다. 기능별 테스트를 분리해 326개 자동 테스트로 주요 동작을 검증했습니다.

...
| 최신 CI 자동 테스트 결과 | 611 passed in 4.51s, 0 skipped, 0 failed |
...
최신 GitHub Actions CI에서는 Alembic 마이그레이션 적용 후 `611 passed in 4.51s`, `0 skipped`, `0 failed`를 확인했습니다. 로컬에서 skipped됐던 PostgreSQL 통합 테스트 25개도 CI에서는 마이그레이션이 적용된 테스트 DB를 사용해 모두 실행되었으며, 이어진 Streamlit 시작 검사에서 Health HTTP `200`을 확인했습니다.
...
최신 CI에서 PostgreSQL 마이그레이션과 전체 테스트 `611 passed in 4.51s`, `0 skipped`, `0 failed`를 확인하였습니다. 실제 Streamlit 서버의 준비 시간은 3초였으며 Health HTTP `200`과 `ok` 응답을 반환하였습니다. 검사 후 프로세스도 정상 종료되었고 전체 Job 실행 시간은 약 59초였습니다.
...
CatalogGuard Lite는 상품 운영자가 CSV 업로드만으로 상품 데이터 품질을 확인할 수 있는 검수 앱입니다. 업로드 검증, 원본 보존형 개인정보 마스킹 미리보기, 중복 상품 탐지, 가격 이상치 탐지, 상품명과 카테고리 불일치 탐지, 필터와 독립된 전체 결과 통계, 결과 필터링, CSV 다운로드까지 하나의 흐름으로 구성했습니다. PostgreSQL에 검수 결과를 저장하고 검색 조건별 이력 조회, 상세 확인과 CSV 다운로드까지 이어지도록 구현했습니다. GitHub Actions CI에서 PostgreSQL 통합 테스트를 포함한 전체 자동 테스트 611개를 실행해 모두 통과했으며, 실제 Streamlit 서버의 시작과 Health HTTP `200` 응답도 확인했습니다.

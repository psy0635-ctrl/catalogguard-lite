...
| CI 검증 범위 | Alembic 마이그레이션, 전체 pytest |
| 필수 컬럼 | 9개 |
| 선택 컬럼 | 2개 |
| 등록된 검수 규칙 함수 | 10개 |
| 샘플 CSV 상품 수 | 5개 |
| 샘플 CSV 검수 결과 | 오류 6건, 주의 0건 |
| 최초 CI 자동 테스트 결과 | 최초 CI 확인 결과: 588 passed in 4.47s, 0 skipped, 0 failed |
...
로컬에서는 `TEST_DATABASE_URL`을 설정하지 않은 상태로 전체 테스트를 실행해 다음 결과를 확인했습니다. 이때 PostgreSQL 연결 및 저장 통합 테스트 25개는 skipped 처리됩니다.

```text
563 passed, 25 skipped, 0 failed, 1 warning
```

GitHub Actions CI에서는 `main` 브랜치 push 또는 `main` 대상 pull request마다 일회성 PostgreSQL 18 서비스 컨테이너를 시작합니다. 서비스 컨테이너는 workflow 실행 중에만 사용할 테스트용 PostgreSQL이며 Railway나 운영 DB와 분리됩니다. CI는 이 DB에 `DATABASE_URL`과 `TEST_DATABASE_URL`을 설정하고 Alembic 마이그레이션을 적용한 뒤, 단위 테스트와 PostgreSQL 통합 테스트를 포함한 전체 pytest를 한 번 실행합니다.
...
커밋 `50467ac9fb8bcc2cf7bd4d5182dc3c99a4bb5a31`의 최초 CI 확인 결과는 `588 passed in 4.47s`, `0 skipped`, `0 failed`입니다. 로컬에서 skipped됐던 PostgreSQL 통합 테스트 25개도 CI에서는 마이그레이션이 적용된 테스트 DB를 사용해 실행되며, 실패 내용은 GitHub Actions 실행 화면에서 확인할 수 있습니다.
...
CI는 테스트 전에 `20260703_0001`, `20260705_0002` Alembic 마이그레이션을 적용합니다. 그 결과 최초 CI에서 로컬 단위 테스트와 PostgreSQL 연결·저장 통합 테스트를 합친 전체 588개가 skipped와 실패 없이 통과했습니다.
...
CatalogGuard Lite는 상품 운영자가 CSV 업로드만으로 상품 데이터 품질을 확인할 수 있는 검수 앱입니다. 업로드 검증, 원본 보존형 개인정보 마스킹 미리보기, 중복 상품 탐지, 가격 이상치 탐지, 상품명과 카테고리 불일치 탐지, 결과 필터링, CSV 다운로드까지 하나의 흐름으로 구성했습니다. PostgreSQL에 검수 결과를 저장하고 검색 조건별 이력 조회, 상세 확인과 CSV 다운로드까지 이어지도록 구현했습니다. GitHub Actions CI에서 PostgreSQL 통합 테스트를 포함한 전체 자동 테스트 588개를 실행했으며 모두 통과했습니다.

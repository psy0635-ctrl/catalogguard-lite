...
**D. ETL 품질 관찰 · 상품 동기화 차이** — ETL 품질 관찰은 전용 Chromium E2E(`tests/e2e/test_etl_quality_observability_browser_e2e.py`)로 확인한다. local disposable PostgreSQL에서 E2E 전용 공급사 품질 batch 두 건을 만든 뒤, operator 로그인 → 공급사 선택 → 최신/직전 Reject 비율·`%p` 변화량·방향·주요 오류 코드·관찰 batch를 실제 FastAPI·Streamlit에서 확인하고 DB/API 응답도 대조한다. fixture는 생성한 정확한 batch ID만 `finally`에서 정리한다. 상품 동기화 차이도 전용 Chromium E2E(`tests/e2e/test_catalog_reconciliation_browser_e2e.py`)로 확인한다. synthetic staging 3건과 운영 catalog 3건을 local disposable PostgreSQL에 만든 뒤, operator 로그인 → batch 검색·선택 → 상세 조회 → 상태·변경 필드·reject 안내를 실제 FastAPI·Streamlit에서 확인하고 DB/API 응답을 대조한다. 이 보고서는 조회 전용이며 `이번 배치 미관측`은 삭제 또는 판매 종료 판정이 아니다. fixture는 정확한 PK만 FK-safe 순서로 정리한다.
